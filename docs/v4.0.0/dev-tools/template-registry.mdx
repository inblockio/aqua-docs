...
A deployed registry is available, without SLA, at <a href="https://aqua-registry.inblock.io">aqua-registry.inblock.io</a>. It can be used to publish and retrieve community-developed templates. Because it runs under active development and experimental conditions, stored templates may be wiped without notice — keep your signed registration trees so you can republish.
</Callout>
...
- **`pins` is a lockfile** mapping qualified names to bare-digest hashes:
...
Publishers sign a **feed head**: a commitment to their own feed — how many of their entries exist, and a Merkle root over them (RFC 6962-style prefixes, with each entry's ordinal inside the leaf so reordering changes the root). A subscriber verifies the head on every sync and fails closed: withholding, reordering or substitution, rollback, equivocation, and the disappearance of a previously seen head all abort the import without advancing the cursor.
...
The registry is the **only sanctioned distribution channel** for this family — see [Agent Audit Trails](/docs/v4.0.0/dev-tools/agent-audit-trails) for how the templates are used, and `seed/audit-set-v1/` in the repository for replaying the committed set into your own registry.
...
- [Agent Audit Trails](/docs/v4.0.0/dev-tools/agent-audit-trails) — the audit template family end to end
- [Templates](/docs/v4.0.0/schema-reference/templates) — template identity, hashing, and derivation
- [Quickstart](/docs/v4.0.0/quickstart) — first steps with the published components
