...
Revisions whose `revision_type` names a timestamp foundation template are classified as timestamp revisions, but timestamping — creation and provider integration — is part of the full protocol, not the published core profile. A core verifier still checks a timestamp revision's batch inclusion proof when it can; unavailable timestamp capability fails under the `strict` policy and warns under `offline`.
</Callout>
...
Full rules: [Hashing](/docs/v4.0.0/schema-reference/hashing).
...
Full rules: [Hashing](/docs/v4.0.0/schema-reference/hashing) and [Selective Disclosure](/docs/v4.0.0/schema-reference/selective-disclosure).
...
Full rules: [Templates](/docs/v4.0.0/schema-reference/templates) and the [SDK Core](/docs/v4.0.0/dev-tools/sdk-core) `export_tree` API.

## See Also

- [Quick Start](/docs/v4.0.0/quickstart) — create, sign, and verify a tree in one program
- [Schema Reference](/docs/v4.0.0/schema-reference/introduction) — the readable wire-format reference
- [Data Model](/docs/v4.0.0/schema-reference/data-model) — revisions and trees in full detail
- [SDK Core](/docs/v4.0.0/dev-tools/sdk-core) — the Rust API implementing these concepts
