...
4. `compositional_links`, when present, is non-empty; each entry carries a full multihash `hash` and a string `role`. Entries are never resolved, and unknown roles are never a reason to reject.
...
In `aqua-rs-sdk-core`, this is surfaced as two `Aquafier` methods: `link_aqua_tree` appends an anchor revision whose `structural_links` carry the tip hashes of the supplied trees, and `verify_aqua_tree_with_linked_trees` runs the cross-tree verification described above. See [SDK Core](/docs/v4.0.0/dev-tools/sdk-core).
...
- [SDK Core](/docs/v4.0.0/dev-tools/sdk-core) — `link_aqua_tree` and `verify_aqua_tree_with_linked_trees` in practice
