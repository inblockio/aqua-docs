...
| `method` | string | REQUIRED | MUST be `"scalar"` or `"tree"`. Selects the hashing method ([Hashing](/docs/v4.0.0/schema-reference/hashing)). |
...
**Notes.** `revision_type` names the `anchor_template` template (bare digest `0x479a30…`). It has no `previous_revision`, making it a genesis anchor; its single structural link names the `file` template that types the tree's content. The full canonicalization walk-through for this exact revision is on the [Hashing](/docs/v4.0.0/schema-reference/hashing) page and in the specification.
...
- [Hashing](/docs/v4.0.0/schema-reference/hashing) — how revision hashes and links are computed
