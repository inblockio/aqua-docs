...
All stakeholders can verify the current state and history of products in real-time. Hash-linked, signed revisions fix the order of supply chain events as they occur; blockchain timestamping is available with the full SDK, scheduled for later publication.
...
- The chain preserves the order of both signatures
- Prevents disputes about transfer content or order
...
Automated recorders and AI agents can sign their own contributions, producing verifiable machine provenance — see [Agent Audit Trails](/docs/v4.0.0/dev-tools/agent-audit-trails).
...
### Scalability
- Object revisions are lightweight, one Aqua chain per product
...
- Custom, template-defined revision types for unique requirements
...
A: No. Verification of chains works offline using cryptographic signatures and hashes. Blockchain and TSA timestamping is defined by the full protocol and ships with the full SDK, scheduled for later publication.
